...
class ResponsablesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @responsable.save
        format.html { redirect_to @responsable, notice: "Le responsable a été créé avec succès." }
  ...
  def update
    ...
    respond_to do |format|
      if @responsable.update_attributes(params[:responsable])
        format.html { redirect_to @responsable, notice: "Le responsable a été mis à jour avec succès." }
